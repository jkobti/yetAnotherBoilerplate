# YetAnotherBoilerplate Admin Frontend Helm Chart

Admin-facing web frontend chart for the YetAnotherBoilerplate project. Deployable to local Kubernetes (kind) and cloud clusters (GKE, EKS, etc.).
...
```bash
# Build the admin image locally (Flutter web build → NGINX)
make build-admin

# Create and start a local kind cluster (if not already running)
make kind-up

# Deploy the admin chart (currently disabled by default)
...
kubectl get pods -n apps -l app.kubernetes.io/name=admin
kubectl logs -n apps -l app.kubernetes.io/name=admin
```
...
| `replicaCount`     | `1`                           | Number of admin pod replicas                                    |
| `image.repository` | `yetanotherboilerplate/admin` | Docker image repository                                         |
...
| `ingress.hosts[0].host`          | `admin.local.dev` | Ingress hostname                                  |
...
helm install yab-admin charts/admin \
  ...
  --set ingress.hosts[0].host=admin.example.com
```
...
Ensure your admin frontend responds with `200 OK` on the root `/` path.

## Cleanup

```bash
# Uninstall the Helm release
helm uninstall yab-admin -n apps
...
- **Local development**: Use `image.pullPolicy=IfNotPresent` and build images locally with `make build-admin`.
...
- **Access control**: Consider using network policies or ingress authentication to restrict access to admin interface.
...
Check logs:
```bash
kubectl logs -n apps -l app.kubernetes.io/name=admin --tail=50
```
